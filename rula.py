...
class RULA:
    ...
    def __call__(self, poses, add_info):
        ...
        for ii in range(len(poses)):
            ...
            # Group A
            group_a_score_L, group_a_score_R, group_a_list = self.group_a(pose, add_info)
            group_a_score_L = (
                group_a_score_L
                + add_info["RULA"]["A_Muscle_use_L"]
                + add_info["RULA"]["A_Load/Force_L"]
            )
            group_a_score_R = (
                group_a_score_R
                + add_info["RULA"]["A_Muscle_use_R"]
                + add_info["RULA"]["A_Load/Force_R"]
            )
            ...
            group_b_score = (
                group_b_score + add_info["RULA"]["B_Muscle_use"] + add_info["RULA"]["B_Load/Force"]
            )
            ...
            data = {"score": final_score, "log_score": group_a_list + group_b_list}
            results.append(data)
    ...
    def group_a(self, pose, add_info):
        ...
        upper_arm += self.upper_arm_bending(pose, add_info)
        upper_arm += self.shoulder_rise(pose)
        upper_arm += self.upper_arm_abducted(pose)
        lower_arm += self.lower_arm_bending(pose)
        lower_arm += self.bent_from_midline_or_out_to_side(pose)
        wrist += self.wrist_bending(pose)
        wrist += self.wrist_side_bending(pose)
        wrist_twist += self.wrist_twist(pose)

        group_a_score_L = self.table_a[upper_arm[0] - 1][lower_arm[0] - 1][wrist[0] - 1][
            wrist_twist[0] - 1
        ]
        group_a_score_R = self.table_a[upper_arm[1] - 1][lower_arm[1] - 1][wrist[1] - 1][
            wrist_twist[1] - 1
        ]
        group_a_list = [
            f"{upper_arm[0]},{upper_arm[1]}",
            f"{lower_arm[0]},{lower_arm[1]}",
            f"{wrist[0]},{wrist[1]}",
            f"{wrist_twist[0]},{wrist_twist[1]}",
        ]
    ...
    def group_b(self, pose, add_info):
        ...
        neck += self.neck_side_bending_twisted(pose)
    ...
    def upper_arm_bending(self, pose, add_info):
        score1, score2 = 0, 0
        ...
        if angle1 > -70 and angle1 < 110:
            if abs(angle2) < 20:
                score1 = 1
            elif angle2 > 20 or (angle2 > -45 and angle2 < -20):
                score1 = 2
            elif angle2 > -90 and angle2 <= -45:
                score1 = 3
            elif angle2 < -90:
                score1 = 4
            else:
            ...
            if abs(angle2) < 20:
                score1 = 1
            elif angle2 > 20 and angle2 < 70:
                score1 = 2
            elif angle2 > 70:
                score1 = 2
            elif angle2 > -70 and angle2 < -20:
                score1 = 4
            elif angle2 < -70:
                score1 = 4
            else:
        ...
        if angle3 > -70 and angle3 < 110:
            if abs(angle4) < 20:
                angle4 = 1
            elif angle4 < -20 or (angle4 > 20 and angle4 <= 45):
                score2 = 2
            elif angle4 > 45 and angle4 <= 90:
                score2 = 3
            elif angle4 > 90:
                score2 = 4
            else:
            ...
            if abs(angle4) < 20:
                score2 = 1
            elif angle4 > -70 and angle4 < -20:
                score2 = 2
            elif angle4 < -70:
                score2 = 2
            elif angle4 > 20 and angle4 < 70:
                score2 = 4
            elif angle4 > 70:
                score2 = 4
            else:
        ...
        self.angle_log[
            "upper_arm_bending"
        ] = f"L {angle1:.1f},{angle2:.1f} R {angle3:.1f},{angle4:.1f}"
        return np.array([score1, score2])

    def shoulder_rise(self, pose):
        score1, score2 = 0, 0
    ...
    def upper_arm_abducted(self, pose):
        score1, score2 = 0, 0
    ...
    def lower_arm_bending(self, pose):
        ...
        if angle1 > -100 and angle1 < -60:
            score1 = 1
        elif angle1 < -100 or (angle1 > -60 and angle1 < 0):
            score1 = 2
        else:
            score1 = 1
        ...
        if angle2 > 60 and angle2 < 100:
            score2 = 1
        elif angle2 > 100 or (angle2 > 0 and angle2 < 60):
            score2 = 2
        else:
            score2 = 1
    ...
    def bent_from_midline_or_out_to_side(self, pose):
        score1, score2 = 0, 0
        angle1 = pose[self.joint_name.index("L_Thorax")][0]

        if angle1 < 10 or (angle1 > -45 and angle1 < -10):
            score1 = 0
        elif angle1 > 10 or angle1 < -45:
            score1 = 1
        else:
            score1 = 0
        ...
        if angle2 > -10 or (angle2 > 10 and angle2 < 45):
            score2 = 0
        elif angle2 < -10 or angle2 > 45:
            score2 = 1
        else:
            score2 = 0
    ...
    def wrist_bending(self, pose):
        ...
        if abs(angle1) < 1:
            score1 = 1
        elif abs(angle1) > 1 and abs(angle1) < 15:
            score1 = 2
        elif abs(angle1) > 15:
            score1 = 3
        else:
        ...
        if abs(angle2) < 1:
            score2 = 1
        elif abs(angle2) > 1 and abs(angle2) < 15:
            score2 = 2
        elif abs(angle2) > 15:
            score2 = 3
        else:
    ...
    def neck_side_bending_twisted(self, pose):
        ...
        if abs(angle1) < 10 and abs(angle2) < 10:
            return 0
        elif abs(angle1) > 10 or abs(angle2) > 10:
            return 1
        else:
            return 0
